  ...
  <admin-layout>
    <v-banner class="mb-4">
      <div class="d-flex flex-wrap justify-space-between">
        <h5 class="text-h5 font-weight-bold">Kategori</h5>
      ...
      <v-card>
        ...
        <v-card-text class="pt-4">
          <v-text-field
            ...
            label="Kategori"
            :error-messages="form.errors.categoryName"
            type="text"
            outlined
            dense
          />
...
export default {
  ...
  computed: {
    formTitle() {
      return this.isUpdate ? "Ubah Kategori" : "Buat Kategori";
    },
  },
  watch: {
  ...
  methods: {
    updateData() {
      ...
      this.$inertia.get("/category", this.params, {
        preserveState: true,
        preverseScroll: true,
        onSuccess: () => {
    ...
    destroy() {
      this.form.delete(route("category.destroy", this.itemId), {
        preverseScroll: true,
        onSuccess: () => {
    ...
    submit() {
      if (this.isUpdate) {
        this.form.put(route("category.update", this.itemId), {
          preverseScroll: true,
          onSuccess: () => {
